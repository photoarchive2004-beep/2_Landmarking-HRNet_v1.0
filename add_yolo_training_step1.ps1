...
$trainYoloPath = Join-Path $root "scripts\train_yolo.py"
$trainerMenuPath = Join-Path $root "scripts\trainer_menu.py"

if (!(Test-Path $trainerMenuPath)) {
    throw "trainer_menu.py not found: $trainerMenuPath"
}

# -------------------------------
# 1. Создаём scripts\train_yolo.py
# -------------------------------
$trainYoloCode = @'
...
Set-Content -Path $trainYoloPath -Value $trainYoloCode -Encoding UTF8
Write-Host "Created/updated scripts\train_yolo.py"


# -------------------------------
# 2. Патч run_train_yolo в trainer_menu.py
# -------------------------------
$trainerText = Get-Content $trainerMenuPath -Raw -Encoding UTF8
...
if ($trainerText -notlike "*Placeholder for YOLO training*") {
    Write-Host "[WARN] Placeholder run_train_yolo block not found in trainer_menu.py. Patch NOT applied." -ForegroundColor Yellow
} else {
    $trainerText = $trainerText.Replace($oldBlock, $newBlock)
    Set-Content -Path $trainerMenuPath -Value $trainerText -Encoding UTF8
    Write-Host "Patched run_train_yolo in scripts\trainer_menu.py"
}
...
git add "scripts/train_yolo.py" "scripts/trainer_menu.py"

$commitMsg = "Add YOLO training (action 1) according to ТЗ-YOLO v1.0"
...
Write-Host "Done. YOLO training code added and pushed." -ForegroundColor Green
